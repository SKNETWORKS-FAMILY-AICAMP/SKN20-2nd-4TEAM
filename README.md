...
| 최유정 | 이번 프로젝트를 통해 실제 데이터를 바탕으로 학생 이탈 예측 모델을 성공적으로 구현했으며, 데이터 분석부터 웹 서비스 구현까지 머신러닝 파이프라인 전반을 경험할 수 있었습니다. 특히, 팀원들의 전문성과 적극적인 협업 덕분에 데이터 불균형 문제 해결과 성능 최적화라는 난관을 극복하고 기대 이상의 완성도 높은 결과물을 만들어낼 수 있었던 것 같습니다. |
